...
func (sfi *ServerForInner) authOverdueCheck() {
	...
	for {
		...
		sfi.tempConnList.Range(func(key, value interface{}) bool {
			...
			if timeNow-innerConn.Created > 28800 && innerConn.Status == connection.StatusInit {
				// 超时没有认证的连接关闭并释放资源
...
func main() {
	...
	c := &OutConfig{}
	confile:=flag.String("config","outer.config.json","config 文件路径")
	flag.Parse()
	err := JsonConfig.Load(*confile, c)
	log.Println("start out service file" + *confile)
	if err != nil {
		panic("can not parse config file:"  + *confile)
	}
	// 启动内部服务
